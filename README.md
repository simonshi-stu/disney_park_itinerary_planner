...
项目处于“存储基础与 ingestion source-envelope 已实现”阶段。治理基础、数据契约、PostgreSQL schema、目标 normalizer replay、只读 parity 报告、重放手册和实时 source-envelope use case 已建立；下一步 collector 双写需要云凭证与首次 hosted restore 人工门禁，live parity 仍需对象存储 bucket 与目标数据库中的完整回填。现有静态网站、Node.js 数据采集器和 GitHub Actions 继续运行，API、forecast、正式切换均尚未开始。
