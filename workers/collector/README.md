...
When `DATABASE_URL` is available, `run-dual-write.mjs` also upserts `ingestion.source_health`. It runs only for a newly written snapshot; collection skips do not re-archive the previous snapshot or create a false healthy run. A collector failure records an outage without reading the previous snapshot. It records source and hosted failure metadata even when raw hosted upload fails; source-health persistence errors are returned as `source_health.status=failed` and signal a failed job after the Git path is preserved.
